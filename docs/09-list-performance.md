...
> **측정값은 2026-09-23 에 PostgreSQL 18.6 에서 다시 잰 것이다.** 처음에는 16.15 에서 쟀는데,
> 9단계에서 배포와 버전을 맞추며 로컬을 올렸다. 결론(인덱스가 실행 계획을 바꾼다, 커서는 깊이와
> 무관하다, 쿼리 수가 상수가 된다)은 그대로였고 숫자만 조금 움직였다.
...
| | 값 |
|---|---|
| 목록 `limit=20` HTTP 중앙값 | 17.5 ms |
| 목록 1회에 나가는 쿼리 | 글 20건이면 **22회** (2 + N) |
| 정렬 쿼리 실행 계획 | `Seq Scan` + `Sort`, Execution Time **1.922 ms** |
...
| `OFFSET 1000` | 1,020 | 0.215 ms |
| `OFFSET 5000` | 5,020 | 0.725 ms |
| `OFFSET 9980` | **10,000** | 1.504 ms |
| 커서로 같은 자리 | **19** | 0.055 ms |

**읽은 행 수를 보는 것이 핵심이다.** offset 은 건너뛴 행도 읽는다. 9980번째부터 20건을 받으려고
10,000행을 읽는다. 커서는 어디서든 20행이다 — 그래서 깊이와 무관하게 일정하다.

offset 방식은 "총 몇 페이지" 를 알려면 `COUNT(*)` 를 한 번 더 내야 한다. 이 데이터에서 **1.259 ms** 다.
...
| 요청 | 전 | 후 |
|---|---|---|
| 목록 `limit=20` | 17.5 ms | 15.3 ms |

**여기가 이 단계에서 가장 정직한 숫자다.** DB 시간은 39배 줄었는데 HTTP 는 2 ms 움직였다.
...
데이터: 글 10,000건 · 댓글 50,000건 · 유저 20명. **PostgreSQL 18.6.**

| 무엇 | 어떻게 | 결과 |
|---|---|---|
| 쿼리 수 상수 | `PRISMA_LOG=query` 로 `limit=1·20·50` | 전부 **2회** |
| 인덱스 전후 | `EXPLAIN ANALYZE`, 인덱스를 떼었다 붙이며 | 1.922 ms → **0.049 ms**, `Seq Scan` → `Index Only Scan` |
| HTTP 전후 | 15회 중앙값 | 17.5 → 15.3 ms |
| offset 대 커서 | `EXPLAIN ANALYZE`, 깊이 0·1000·5000·9980 | 읽은 행 20 → 10,000 대 커서 19 고정 |
...
## PostgreSQL 18 에서 달라진 것

버전을 올리며 `EXPLAIN` 출력이 바뀐 것을 알았다. 측정 스크립트가 값을 못 뽑아서 드러났다.

| | 17 까지 | 18 |
|---|---|---|
| 행 수 표기 | `rows=21` | `rows=21.00` (소수) |
| `Buffers:` | `BUFFERS` 옵션을 줘야 나온다 | `EXPLAIN ANALYZE` 기본 출력 |

로그를 정규식으로 긁는 스크립트는 **DB 메이저 버전이 바뀌면 같이 깨진다.**

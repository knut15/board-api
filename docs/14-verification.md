...
| `smoke-deployed.sh` | 배포 직후 — 전 구간이 이어졌는지 (**읽기만**) | 5 | 아니오 |
...
대신 배포 뒤에는 `smoke-deployed.sh` 가 돈다. 읽기만 하는 5가지라 매번 돌아도 데이터가 안 는다
(→ [15-continuous-deploy.md](./15-continuous-deploy.md)).

## 아직 안 한 것

- **단위 테스트가 없다.** 전부 HTTP 를 통과하는 검사다. 빠르지는 않지만(43가지에 수십 초)
  "계약이 지켜지는가" 를 보는 데는 이쪽이 맞다.
- ~~배포는 여전히 손으로 한다.~~ → [15-continuous-deploy.md](./15-continuous-deploy.md) 에서 이었다.
  검사가 통과한 `main` push 만 자동으로 올라간다.

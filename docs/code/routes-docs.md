...
배포된 서버에는 공개 도메인이 없다. 그래서 이 `/docs` 는 그대로 두면 **로컬에서만 보인다.**

선택지가 둘이었다 — `server` 에 공개 도메인을 붙이거나(API 전체가 같이 공개된다), Next 가 `/docs`
요청을 대신 받아 내부 주소로 넘기거나(서버는 계속 내부에만 남는다). 9.3 에서 "밖에 나가는 것은
web 하나" 로 정했으므로 **뒤쪽을 골랐다** → [`web-docs-proxy.md`](./web-docs-proxy.md).

이 파일은 그 결정을 모른다. 화면과 명세를 만들어 주는 일만 하고, 누가 그것을 밖으로 나르는지는
알 필요가 없다.

## 다음 단계에서 어떻게 바뀌는가

`Try it out` 은 공개 주소에서 동작하지 않는다. `servers` 에 로컬 주소만 있어서인데 그건 의도다 —
배포 주소를 넣으면 버튼 한 번이 운영 DB 에 글을 쓴다.

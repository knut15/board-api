...
화면은 고쳤지만 `logout` 은 여전히 `localStorage` 의 액세스 토큰만 지우고 있었다.
**리프레시 쿠키는 그대로 남았다.** 같은 브라우저에서 `POST /auth/refresh` 를 부르면 새 토큰이
나왔다 — 화면상으로는 로그아웃인데 서버가 보기엔 아직 로그인한 사람이었다.

쿠키는 `httpOnly` 라 자바스크립트가 읽지도 지우지도 못한다. **서버만 지울 수 있다.**
그래서 로그아웃이 요청이 됐다(`POST /auth/logout` → `web-auth-proxy.md`).
...
**기다리지 않는다.** 유스케이스가 토큰을 먼저 지우고 그다음 서버를 부르므로, 화면은 요청이
떠나기 전에 이미 바뀐다. 왕복을 기다리면 배포 환경에서 수백 밀리초 동안 "눌렀는데 그대로" 가
되고, 그게 바로 고치던 증상이다.

**실패를 삼키지 않는다.** 요청이 실패해도 이 브라우저는 이미 로그아웃 상태다 — 액세스 토큰이
없으니 아무것도 못 한다. 남는 것은 서버 쪽 쿠키뿐이라 사용자에게 띄울 말이 없다. 그래도 콘솔에는
남긴다. 조용히 없던 일로 만들면 서버가 며칠째 로그아웃을 못 받고 있어도 아무도 모른다.

여기서 지워지는 것은 이 브라우저의 쿠키까지다. 새어 나간 토큰을 무효로 만드는 것은 서버가
상태를 들어야 하는 일이고, 왜 지금 하지 않았는지는 `routes-auth.md` 에 적었다.
...
최신순/오래된 순 토글 하나다. 눌러 보면 첫 글이 가장 최근 글에서 가장 오래된 글로 바뀐다.
캐시 키도 `{"sort":"createdAt:desc"}` 와 `{"sort":"createdAt:asc"}` 둘로 갈린다 —
정렬을 되돌리면 앞서 받은 목록이 그대로 돌아오고 다시 받지 않는다.
